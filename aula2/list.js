...
//2.Processe a lista de álbuns gerando uma nova lista, com textos no formato:
//nome (cantor, ano). Use desestruturação.

newAlbum = [];

for (const album of albuns){
    const {nome, cantor,ano} = album
    const texto = `${nome}(${cantor},${ano})`;
    newAlbum.push(texto);
}
console.log(newAlbum)
//3. Processe a lista albuns gerando um objeto onde as propriedades é o nome do cantor e os valores são a quantidade de álbuns que aquele cantor possui.

albunsCantores = {}

for(const album of albuns){
    const cantor = album.cantor;
    if (albunsCantores[cantor]){
        albunsCantores[cantor]++
    }else{
        albunsCantores[cantor] = 1
    }
}
